...
public class NgnProxyVideoConsumerGL extends NgnProxyVideoConsumer{
    ...
    private int bufferCopiedCallback(long nCopiedSize, long nAvailableSize) {
		...
		if (false) {
			synchronized (mPreview) {
				if (mVideoFrame != null && nAvailableSize > 100000) {

//					byte[] data = mVideoFrame.array();
...
//					}
					...
					try {
						String root = Environment.getExternalStorageDirectory().toString() + "/req_images";
...
//						byte[] gg = mVideoFrame.array();
//						int length = gg.length;
//						Log.d("GGGG", "mVideoFrame.capacity() = " + mVideoFrame.capacity());
//						Log.d("GGGG", "ggsize = " + length);
//						for (int i = 0; i < length*4/6; i++) {
//							gg[i] = 0x0;
//						}
//						for (int i = length*4/6; i < length*5/6; i++) {
//							gg[i] = 0x0;
//						}
//						for (int i = length*5/6; i < length; i++) {
//							gg[i] = 0x0;
//						}

						byte[] gg = mVideoFrame.array();
						byte[] qq = NgnImageUtils.I420ToNv21(gg, mWidth, mHeight);

						YuvImage image = new YuvImage(qq, ImageFormat.NV21, mWidth, mHeight, null);
						image.compressToJpeg(new Rect(0, 0, image.getWidth(), image.getHeight()), 90, fo);
//						fo.write(mVideoFrame.array());
	...
	static class NgnProxyVideoConsumerGLPreview extends GLSurfaceView implements GLSurfaceView.Renderer{
		...
		private ByteBuffer scaleBuffer;
...
//				"} ";
		...
		public void setBuffer(ByteBuffer buffer, int bufferWidth, int bufferHeight){
			...
			if (bufferWidth % 8 != 0) {
				Log.d(TAG, "Detect width/height need to be aligned");
				scaledWidth = (bufferWidth - bufferWidth % 8);
				scaledHeight = (bufferHeight - bufferHeight % 8);
				scaleBuffer = ByteBuffer.allocateDirect(scaledWidth * scaledHeight * 3 / 2);
			} else {
			...
			if (mBuffer != null) {
				synchronized (this) {
					...
					if (false) {
						if (mBuffer.capacity() > 100000) {
							try {
								...
								byte[] gg = mBuffer.array();
								byte[] qq = NgnImageUtils.I420ToNv21(gg, mBufferWidthY, mBufferHeightY);

								YuvImage image = new YuvImage(qq, ImageFormat.NV21, mBufferWidthY, mBufferHeightY, null);
								image.compressToJpeg(new Rect(0, 0, image.getWidth(), image.getHeight()), 90, fo);
					...
					if (scaledWidth != -1 && scaledHeight != -1) {
						...
						byte[] newNV21 = NgnImageUtils.getNV21(scaledWidth, scaledHeight, resized);
						byte[] I420 = NgnImageUtils.NV21ToI420(newNV21, scaledWidth, scaledHeight);
						scaleBuffer.position(0);
						scaleBuffer.put(I420);

						GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
						GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mTextureY[0]);
						GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, scaledWidth, scaledHeight,
								0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, scaleBuffer.position(mBufferPositionY));
						GLES20.glUniform1i(muSamplerYHandle, 0);

						GLES20.glActiveTexture(GLES20.GL_TEXTURE1);
						GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mTextureU[0]);
						GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, scaledWidth/2, scaledHeight/2,
								0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, scaleBuffer.position(scaledWidth*scaledHeight));
						GLES20.glUniform1i(muSamplerUHandle, 1);

						GLES20.glActiveTexture(GLES20.GL_TEXTURE2);
						GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mTextureV[0]);
						GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, scaledWidth/2, scaledHeight/2,
								0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, scaleBuffer.position(scaledWidth*scaledHeight + scaledWidth*scaledHeight/4));
