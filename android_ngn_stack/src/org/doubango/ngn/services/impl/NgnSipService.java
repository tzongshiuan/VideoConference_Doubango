...
import org.doubango.tinyWRAP.InfoEvent;
...
public class NgnSipService extends NgnBaseService implements INgnSipService,
	...
	public static boolean IS_ENABLE_STUN;
	public static boolean IS_AV_CHAT_PORT_APART;

	private static final int DEBUG_LEVEL_TRACE = 6;
	private static final int DEBUG_LEVEL_DEBUG = 6;
	private static final int DEBUG_LEVEL_INFO = 6;
	private static final int DEBUG_LEVEL_WARN = 6;
	private static final int DEBUG_LEVEL_ERROR = 6;
	private static final int DEBUG_LEVEL_FATAL = 6;
	private final int debugLevel = DEBUG_LEVEL_INFO;

	private static final String STUN_SERVER = "192.168.11.26";
	private static final int STUN_PORT = 3478;
	private static final String STUN_USERNAME = "gorilla";
	private static final String STUN_PASSWORD = "gorillakm";

	private NgnRegistrationSession mRegSession;
	private NgnSipStack mSipStack;

	private NgnRegistrationSession mChatRegSession;
	private NgnSipStack mChatSipStack;
	...
	public boolean stop() {
		...
		if (IS_AV_CHAT_PORT_APART) {
			if (mChatSipStack != null && mChatSipStack.getState() == STACK_STATE.STARTED) {
				ret = mChatSipStack.stop();
			}
		}
	...
	@Override
	public NgnSipStack getChatStack() {
		if (IS_AV_CHAT_PORT_APART) {
			return mChatSipStack;
		} else {
			return mSipStack;
		}
	}
	...
	private boolean initSipStack(Context context) {
		if (mSipStack == null) {
			...
			mSipStack.setDebugLevel(debugLevel);
        ...
        Log.d(TAG, "Is enable STUN: " + IS_ENABLE_STUN);
		if (IS_ENABLE_STUN) {
			mSipStack.setSTUNEnabled(true);
			mSipStack.setSTUNEnabledForICE(false);
		...
		Log.d(NgnSipService.TAG, "IS_AV_CHAT_PORT_APART = " + IS_AV_CHAT_PORT_APART);
	...
	private boolean initChatSipStack(Context context) {
		if (mChatSipStack == null) {
			mChatSipStack = new NgnSipStack(mSipCallback, mPreferences.getRealm(),
					mPreferences.getIMPI(), mPreferences.getIMPU());
			mChatSipStack.setDebugCallback(mDebugCallback);
			SipStack.setCodecs_2(mConfigurationService.getInt(
					NgnConfigurationEntry.MEDIA_CODECS,
					NgnConfigurationEntry.DEFAULT_MEDIA_CODECS));
		} else {
			if (!mChatSipStack.setRealm(mPreferences.getRealm())) {
				Log.e(TAG, "Failed to set realm");
				return false;
			}
			if (!mChatSipStack.setIMPI(mPreferences.getIMPI())) {
				Log.e(TAG, "Failed to set IMPI");
				return false;
			}
			if (!mChatSipStack.setIMPU(mPreferences.getIMPU())) {
				Log.e(TAG, "Failed to set IMPU");
				return false;
			}
		}
		...
		// set the Password
		mChatSipStack.setPassword(mConfigurationService.getString(
				NgnConfigurationEntry.IDENTITY_PASSWORD,
				NgnConfigurationEntry.DEFAULT_IDENTITY_PASSWORD));
		// Set AMF
		mChatSipStack.setAMF(mConfigurationService.getString(
				NgnConfigurationEntry.SECURITY_IMSAKA_AMF,
				NgnConfigurationEntry.DEFAULT_SECURITY_IMSAKA_AMF));
		// Set Operator Id
		mChatSipStack.setOperatorId(mConfigurationService.getString(
				NgnConfigurationEntry.SECURITY_IMSAKA_OPID,
				NgnConfigurationEntry.DEFAULT_SECURITY_IMSAKA_OPID));

		// Check stack validity
		if (!mChatSipStack.isValid()) {
			Log.e(TAG, "Trying to use invalid stack");
			return false;
		}
		...
		// Set STUN information
		if (IS_ENABLE_STUN) {
			mChatSipStack.setSTUNEnabled(true);
			mChatSipStack.setSTUNEnabledForICE(false);
			if (mConfigurationService.getBoolean(
					NgnConfigurationEntry.NATT_STUN_DISCO,
					NgnConfigurationEntry.DEFAULT_NATT_STUN_DISCO)) {
				final String realm = mPreferences.getRealm();
				String domain = realm.substring(realm.indexOf(':') + 1);
				int[] port = new int[1];
				String server = mChatSipStack.dnsSrv(
						String.format("_stun._udp.%s", domain), port);
				if (server == null) {
					Log.e(TAG, "STUN discovery has failed");
				}
				Log.d(TAG, String.format("STUN1 - server=%s and port=%d",
						server, port[0]));
				mChatSipStack.setSTUNServer(server, port[0]);// Needed event if null
			} else {
				String server = mConfigurationService.getString(
						NgnConfigurationEntry.NETWORK_PCSCF_HOST, null);
				int port = STUN_PORT;
				Log.d(NgnSipService.TAG, String.format(
						"STUN2 - server=%s and port=%d", server, port));
				mChatSipStack.setSTUNServer(server, port);
				mChatSipStack.setSTUNCred(STUN_USERNAME, STUN_PASSWORD);
			}
		}
		else {
			mChatSipStack.setSTUNEnabled(mConfigurationService.getBoolean(
					NgnConfigurationEntry.NATT_USE_STUN_FOR_SIP,
					NgnConfigurationEntry.DEFAULT_NATT_USE_STUN_FOR_SIP));
			if (mConfigurationService.getBoolean(
					NgnConfigurationEntry.NATT_STUN_DISCO,
		...
		if (!mChatSipStack.setProxyCSCF(mPreferences.getPcscfHost(),
				5060, "UDP",
				mPreferences.getIPVersion())) {
			Log.e(NgnSipService.TAG, "Failed to set Proxy-CSCF parameters");
			return false;
		...
		Log.d(TAG,
				String.format(
						"ChatStack => pcscf-host='%s', pcscf-port='%d', transport='%s', ipversion='%s'",
						mPreferences.getPcscfHost(),
						5060,
						"UDP",
						mPreferences.getIPVersion()));
		...
		if (!mChatSipStack.setLocalIP(mPreferences.getLocalIP())) {
			Log.e(TAG, "Failed to set the local IP");
			return false;
		}
		Log.d(TAG, String.format("Local IP='%s'", mPreferences.getLocalIP()));
		...
		String discoverType = mConfigurationService.getString(
				NgnConfigurationEntry.NETWORK_PCSCF_DISCOVERY,
				NgnConfigurationEntry.DEFAULT_NETWORK_PCSCF_DISCOVERY);
		mChatSipStack.setDnsDiscovery(NgnStringUtils.equals(discoverType,
				NgnConfigurationEntry.PCSCF_DISCOVERY_DNS_SRV, true));

		// enable/disable 3GPP early IMS
		mChatSipStack.setEarlyIMS(mConfigurationService.getBoolean(
				NgnConfigurationEntry.NETWORK_USE_EARLY_IMS,
				NgnConfigurationEntry.DEFAULT_NETWORK_USE_EARLY_IMS));
		...
		if (mConfigurationService.getBoolean(
			...
			String compId = String.format("urn:uuid:%s", UUID.randomUUID()
					.toString());
			mChatSipStack.setSigCompId(compId);
		} else {
			mChatSipStack.setSigCompId(null);
		}
		...
		Log.d(TAG, String.format("TLS - pvk='%s' pbk='%s' ca='%s' verify=%s",
				pvFilePath, pbFilePath, caFilePath, verifyCerts));
		if (!mChatSipStack.setSSLCertificates(pvFilePath, pbFilePath, caFilePath,
				verifyCerts)) {
			Log.e(TAG, "Failed to set TLS certificates");
			return false;
		}
		...
		// Start the Stack
		if (!mChatSipStack.start()) {
			if (context != null
					&& Thread.currentThread() == Looper.getMainLooper()
					.getThread()) {
				Toast.makeText(context, "Failed to start the Chat SIP stack",
						Toast.LENGTH_LONG).show();
			}
			Log.e(TAG, "Failed to start the Chat SIP stack");
			return false;
		}
	...
	public boolean register(Context context) {
		...
		if (!initSipStack(context)) {
			return false;
		}

		if (IS_AV_CHAT_PORT_APART) {
			if (!initChatSipStack(context)) {
				return false;
			}
		}
...
//		}
		...
		if (IS_AV_CHAT_PORT_APART) {
			// Create registration session
			if (mChatRegSession == null) {
				mChatRegSession = new NgnRegistrationSession(mChatSipStack);
			} else {
				mChatRegSession.setSigCompId(mChatSipStack.getSigCompId());
			}
			mChatRegSession.setFromUri(mPreferences.getIMPU());

			if (!mChatRegSession.register()) {
				Log.e(TAG, "[CHAT] Failed to send REGISTER request");
				return false;
			}
		}
	...
	private void broadcastInviteEvent(NgnInviteEventArgs args, String customers) {
		final Intent intent = new Intent(NgnInviteEventArgs.ACTION_INVITE_EVENT);
		intent.putExtra(NgnInviteEventArgs.EXTRA_EMBEDDED, args);
		intent.putExtra(NgnInviteEventArgs.EXTRA_SESSION, customers);
		NgnApplication.getContext().sendBroadcast(intent);
	}
	...
	static class MySipCallback extends SipCallback {
			...
			switch (type){
                ...
                case tsip_i_ect_newcall:
                    ...
                    if (sessionType == twrap_media_type_t.twrap_media_msrp) {
                        ...
                        // Hsuan: tricky way to accept session automatically here
						Log.d("ConcallActivity", "Accept MsrpSession automatically");
                        session.accept();

						NgnMsrpSession msrpSession;
                        if (IS_AV_CHAT_PORT_APART) {
							msrpSession = NgnMsrpSession.takeIncomingSession(mSipService.getChatStack(),
									(MsrpSession) session, message);
						} else {
							msrpSession = NgnMsrpSession.takeIncomingSession(mSipService.getSipStack(),
									(MsrpSession) session, message);
						}
                        if (msrpSession == null){
                        ...
                        mSipService.broadcastInviteEvent(new NgnInviteEventArgs(msrpSession.getId(), NgnInviteEventTypes.INCOMING, msrpSession.getMediaType(), phrase));
							...
							(sessionType == twrap_media_type_t.twrap_media_secondary_video) ||
                ...
                case tsip_i_request:
                    {
                    	...
                    	if(sipMessage != null && session != null && ((mySession = NgnAVSession.getSession(session.getId())) != null)){
                    		if (sipMessage.getRequestType() == tsip_request_type_t.tsip_INVITE) {
                    			// Added by Hsuan
								// Fit the feature that we change encode resolution sent from server dynamically.

								SdpMessage mm = e.getSipMessage().getSdpMessage();
								if (mm != null) {
									String imageAttr = mm.getSdpHeaderAValue("video", "imageattr");
									mSipService.broadcastInviteEvent(new NgnInviteEventArgs(session.getId(), NgnInviteEventTypes.RESOLUTION_UPDATED, ((NgnInviteSession)mySession).getMediaType(), phrase), imageAttr);
								}
							} else if(sipMessage.getRequestType() == tsip_request_type_t.tsip_INFO){
								...
								else if(NgnStringUtils.equals(contentType, NgnContentType.JSON, true)){
									final byte content[] = sipMessage.getSipContent();
									if(content != null) {
										final String values[] = new String(content).split("\r\n");
										mSipService.broadcastInviteEvent(new NgnInviteEventArgs(session.getId(), NgnInviteEventTypes.INCOMING, ((NgnInviteSession)mySession).getMediaType(), phrase),values[0]);
									}
		...
		public int OnMessagingEvent(MessagingEvent e) {
			...
			case tsip_i_message:
				...
				if (content != null && from.indexOf("systemlist") <0) {
					mSipService
							.broadcastMessagingEvent(
				...
				else if(from.indexOf("systemlist") >0){
					mSipService
							.broadcastMessagingEvent(
									new NgnMessagingEventArgs(_session.getId(),
											NgnMessagingEventTypes.Inform, e
											.getPhrase(), content,
											contentType), from,
									NgnDateTimeUtils.now());
				}
