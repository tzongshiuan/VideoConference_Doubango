...
open class UpdateStatusTest {
    ...
    private fun toJsonObject(status: UpdateStatusTest): JSONObject {
        val data = JSONObject()
    ...
    fun getControlChatRoomObject(status: UpdateStatusTest, isEnable: Boolean): JSONObject {
        if (isEnable) {
    ...
    fun getChangeHostObject(status: UpdateStatusTest, hostId: Int): JSONObject {
        status.host = hostId
        return toJsonObject(status)
    }

    fun getControlParticipantObject(status: UpdateStatusTest, id: Int,
    ...
    fun getControlStreamObject(status: UpdateStatusTest, id: Int, streamContent: Int, rtspUrl: String?): JSONObject {
         status.participants?.forEach loop@ { participant ->
             if (participant.id == id) {
     ...
     */
    fun getHostChangeViewObject(status: UpdateStatusTest, mode: Int, leftId: Int, rightId: Int): JSONObject {
